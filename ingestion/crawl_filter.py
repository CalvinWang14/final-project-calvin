...
is_political_domain_udf = F.udf(is_political_domain, BooleanType())
is_political_url_udf = F.udf(is_political_url_path, BooleanType())
...
def main():
    ...
    political_df = df.filter(
        is_political_domain_udf(F.col("url")) |
        is_political_url_udf(F.col("url"))
    ).select(
        "url",
        "fetch_time",
        "warc_filename",
